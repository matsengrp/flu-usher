...
def is_gisaid_date(value):
    ...
    harmless, only quiet -- `scripts/date_filters.py:parse_iso_date` in
    flu-dasm-antigenic-evo parses strict %Y-%m-%d and returns None on failure,
    so an impossible date would drop out of chronumental's node dating and the
    root-to-tip regression with no error, one repository downstream of here.
    Missing precision is normal; an impossible date is a transcription error.
    """
